...
class Categories extends Component {
    ...
    toggle = (e) => {
      ...
      if (e !== null && e !== 0) {
        Meteor.call('getProductCategoryById', e, (error, result) => {
          this.setState({
            data: {
              _id: result._id,
              name: result.name,
    ...
    formUpdateOrCreateCategory = (e) => {
      ...
      if (data._id === null) {
        Meteor.call('createProductCategory', { name: this.inputName.value }, (error, result) => {
        ...
        Meteor.call('updateProductCategory', { id: data._id, name: this.inputName.value }, (error) => {
          if (error) { return alert.show(error.reason, { type: 'error' }); }
            ...
            <Button
              onClick={() => {
                if (confirm('Voulez-vous vraiment supprimer cet élément ?')) {
                  Meteor.call('removeProductCategory', row._id);
      ...
      return (
        <Container id="main-content">
          <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3">
            <h1 className="h2">Gestion des catégories des produits</h1>
